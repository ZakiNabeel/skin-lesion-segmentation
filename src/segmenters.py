...
def adaptive_thresholding(image, block_size=21, C=5):
    ...
    local_mean = convolve2d(gray, kernel)
    
    # Lesions are darker than the local skin average
    mask = (gray < (local_mean - C)).astype(np.uint8)
...
def canny_segmentation(image, low_thresh=20, high_thresh=60):
    ...
    # Connect weak edges to strong ones (Morphological expansion)
    for _ in range(3): 
        strong_dilated = morph_dilate(edges, 3)
        edges[weak & (strong_dilated == 1)] = 1
        weak = weak & ~(edges == 1)
        
    # Convert pure edges to a filled blob for Dice comparison
...
def marr_hildreth_segmentation(image):
    ...
    log_img = convolve2d(gray, log_kernel)
    ...
    # Filter noise based on local variance threshold
...
def manual_combination_segmentation(image):
    ...
    # Apply Threshold (Inverted: Lesions are darker)
    mask = (gray < threshold).astype(np.uint8)
    
    # Clean it up with morphological closing (Dilate then Erode)
    closed = morph_erode(morph_dilate(mask, 15), 15)
    return closed